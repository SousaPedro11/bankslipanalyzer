...
def get_bank_slip_return_service() -> BankSlipReturnNexxeraService:
    return BankSlipReturnNexxeraService()


@router.post("/return/cob")
async def validate_return() -> str:
...
async def validate_return_file(
    ...
    service: BankSlipReturnNexxeraService = Depends(get_bank_slip_return_service),
) -> ReturnFileSchema:
    validate_file_extension(str(file.filename), ".RET")
